...
class CoxProportionalHazard:
    ...
    A wikipedia-supported implementation of the Cox Proportional hazard model
    """
    ...
    def fit(self, df, time_col, event_col):
        """
        Optimizes beta values for all covariates using the breslow-method.
        The b-values get updated using the Newton-Raphson algorithm.

        :param df: Polars df containing time, censored data, and covariates
        :param time_col: string containing name of the column that holds time
        :param event_col: string containing name of the column that holds the events
        :return: polars df containing the beta value and hazard ratio for all covariates
        """
        ...
        for _ in range(0, 1000):
            ...
            for row_full, row_cov in zip(df.iter_rows(named=True), self._wanted_cov.iter_rows()):
                if row_full[event_col] == 1:
                    ...
                    relative_risk = self.__calculate_relative_risk(covariates_risk_set)
    ...
    def plot_beta_value(self):
        """
        Creates a barplot to display the beta values
        :return:
        """
    ...
    def plot_hazard_ratio(self):
        """
        Creates a barplot to display the hazard ratio
        :return:
        """
        temp_data = self._result.sort("beta values")
    ...
    def __calculate_relative_risk(self, risk_set):
